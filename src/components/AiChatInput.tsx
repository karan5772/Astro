...
export default function AiChatInput({
  placeholders = [
    ...
    "What does Venus in my 7th house mean?",
    "Is this a good year for career growth?",
    "Tell me about my day ruler...",
    "What is the significance of a night birth?",
  ],
  onSubmit,
  onChange,
  className,
  ...
  return (
    <div
      className={cn(
        "relative mx-auto h-14 w-full overflow-hidden rounded-xl border border-card-border bg-secondary/80 transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] focus-within:border-primary/50",
        value && "border-primary/30 bg-secondary/90",
    ...
    >
      ...
      <input
        ...
        className={cn(
          "relative z-10 h-full w-full rounded-xl border-none bg-transparent pl-4 pr-16 text-sm tracking-[-0.01em] text-white focus:outline-none sm:pl-6 sm:text-[15px]",
          animating && "text-transparent"
        )}
      />

      {/* animated placeholder */}
      <div className="pointer-events-none absolute inset-0 flex items-center rounded-xl pl-4 sm:pl-6">
      ...
      <button
        ...
        className={cn(
          "absolute right-3 top-1/2 z-20 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-lg bg-primary/20 text-primary border border-primary/30 transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] hover:bg-primary/30",
          value.trim() && !animating
            ? "scale-100 opacity-100 cursor-pointer"
            : "scale-[0.85] opacity-[0.2] cursor-not-allowed"
        )}
      >
        <motion.svg
